...
${BASH_BLUE}██████${BASH_BOLD_WHITE}███${BASH_BLUE}██${BASH_BOLD_WHITE}███${BASH_BLUE}██████${BASH_NC}   Magic Auth Demo
...
get_and_print_domain() {
    ...
    echo -e "\nMAGIC_REDIRECT_URI: $DOMAIN/callback" >> appsecrets.yaml
    echo "Your appsecrets.yaml file has been updated with the redirect URI."

    echo "Setting the redirect URI environment variable..."
    gcloud run services update $SERVICE_NAME --region $REGION --update-env-vars=MAGIC_REDIRECT_URI=$DOMAIN/callback
    print_finished_message $DOMAIN
}
